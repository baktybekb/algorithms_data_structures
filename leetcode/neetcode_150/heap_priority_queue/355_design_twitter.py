from typing import List


class Twitter:

    def __init__(self):
        self.count = 0
        self.follow_map = {}
        self.tweet_map = {}

    def postTweet(self, userId: int, tweetId: int) -> None:
        if userId not in self.tweet_map:
            self.tweet_map[userId] = []
        self.tweet_map[userId].append((self.count, tweetId))
        self.count += 1

    def getNewsFeed(self, userId: int) -> List[int]:
        heap_data = []
        if userId in self.follow_map:
            for followee_id in self.follow_map[userId]:
                if followee_id not in self.tweet_map:
                    continue
                last_idx = len(self.tweet_map[followee_id]) - 1
                count, tweet_id = self.tweet_map[followee_id][last_idx]
                heap_data.append((count, tweet_id, followee_id, last_idx))
        if userId in self.tweet_map:
            last_idx = len(self.tweet_map[userId]) - 1
            count, tweet_id = self.tweet_map[userId][last_idx]
            heap_data.append((count, tweet_id, userId, last_idx))
        news_feed = []
        heap = Heap(heap_data, max_heap)
        while heap and len(news_feed) < 10:
            count, tweet_id, userId, last_idx = heap.remove()
            news_feed.append(tweet_id)
            if last_idx - 1 >= 0 and self.tweet_map[userId]:
                count, tweet_id = self.tweet_map[userId][last_idx - 1]
                heap.insert((count, tweet_id, userId, last_idx - 1))
        return news_feed

    def follow(self, followerId: int, followeeId: int) -> None:
        if followerId not in self.follow_map:
            self.follow_map[followerId] = set()
        self.follow_map[followerId].add(followeeId)

    def unfollow(self, followerId: int, followeeId: int) -> None:
        if followerId not in self.follow_map:
            return
        self.follow_map[followerId].remove(followeeId)


def max_heap(a, b):
    return a > b


class Heap:
    def __init__(self, array, func):
        self.func = func
        self.heap = self.build(array)

    def __len__(self):
        return len(self.heap)

    def is_empty(self):
        return len(self.heap) == 0

    def build(self, array):
        idx = (len(array) - 2) // 2
        for i in reversed(range(idx + 1)):
            self.sift_down(i, array)
        return array

    def sift_down(self, current_idx, array):
        child_one_idx = current_idx * 2 + 1
        end_idx = len(array) - 1
        while child_one_idx <= end_idx:
            child_two_idx = current_idx * 2 + 2 if current_idx * 2 + 2 <= end_idx else -1
            if child_two_idx != -1 and self.func(array[child_two_idx][0], array[child_one_idx][0]):
                swap_idx = child_two_idx
            else:
                swap_idx = child_one_idx
            if self.func(array[swap_idx][0], array[current_idx][0]):
                self.swap(swap_idx, current_idx, array)
                current_idx = swap_idx
                child_one_idx = current_idx * 2 + 1
            else:
                break

    def peek(self):
        return self.heap[0]

    def remove(self):
        self.swap(0, len(self.heap) - 1, self.heap)
        value = self.heap.pop()
        self.sift_down(0, self.heap)
        return value

    def insert(self, value):
        self.heap.append(value)
        self.sift_up(len(self.heap) - 1)

    def sift_up(self, current_idx):
        parent_idx = (current_idx - 1) // 2
        while current_idx > 0 and self.func(self.heap[current_idx][0], self.heap[parent_idx][0]):
            self.swap(parent_idx, current_idx, self.heap)
            current_idx = parent_idx
            parent_idx = (current_idx - 1) // 2

    def swap(self, i, j, array):
        array[i], array[j] = array[j], array[i]

