...
# O(n^2) time | O(n^2) space, n --> length of points list
class Solution:
    def minCostConnectPoints(self, points: List[List[int]]) -> int:
        ...
        for i in range(len(points)):
            ...
            for j in range(len(points)):
                if i == j:
                    continue
                point_map[i].append(j)

        visited, cost = set(), 0
        heap = Heap([(0, 0)], min_heap)
        while len(visited) != len(points):
            distance, point = heap.remove()
            if point in visited:
                continue
            visited.add(point)
            cost += distance
            x, y = points[point]
            for nei in point_map[point]:
                x1, y1 = points[nei]
                if nei in visited:
                    continue
                heap.insert((abs(x - x1) + abs(y - y1), nei))
        return cost
